...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        let Searching = await message.channel.send(":mag_right: Searching Song ...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,"❌ | Some Error Occured With **Patukaran**,Retry Or Try After Some Times.");
        }
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
              return client.sendTime(interaction,"❌ | Some Error Occured With **Patukaran**,Retry Or Try After Some Times.");
            }
